...
@property (strong, nonatomic) MBNetworkManager *netManager;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.netManager = [MBNetworkManager shareManager];
...
- (void)getBookSummary {
    ...
    _summaryTask = [_netManager getWithAPIType:MBAPITypeBookSummary parameter:_readingBook.idField success:^(id response) {
...
- (void)getBookChaptersLink {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        __weak typeof(self) wself = self;
        _chaptersLinkTask = [_netManager getWithAPIType:MBAPITypeChaptersLink parameter:_selectSummary.idField success:^(id response) {
            NSArray *arr = (NSArray *)response;
            if (arr.count > 0) {
...
- (void)getChapterContentWith:(NSUInteger)chapter autoLoad:(BOOL)isAutoLoad{
    ...
    __weak typeof(self) wself = self;
    _chapterTask = [_netManager getWithAPIType:MBAPITypeChapterContent parameter:chapterM.link success:^(id response) {
...
- (void)getChapterContentWith:(NSUInteger)chapter completion:(void (^)())completion failure:(void (^)(NSString *))failure {
    ...
    __weak typeof(self) wself = self;
    _getChapterTask = [_netManager getWithAPIType:MBAPITypeChapterContent parameter:chapterM.link success:^(id response) {
        chapterM.body = [YChapterContentModel adjustParagraphFormat:((YChapterContentModel *)response).body];
        chapterM.isLoad = YES;
        if (wself.settings.isTraditional) {
            [chapterM.traditionalStr class];//check traditionalStr
