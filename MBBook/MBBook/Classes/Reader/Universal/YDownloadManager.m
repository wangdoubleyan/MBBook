...
//  YReader 
//
//  Created by Bing Ma on 2016/12/15.
    ...
    dispatch_once(&onceToken, ^{
        ...
        if ( [[UIDevice currentDevice].systemVersion floatValue] >= 8.0 ) {
            for (NSUInteger i = 0 ; i < queueCount; i++) {
                ...
                queues[i] = dispatch_queue_create("com.Hannb.download", attr);
            }
        } else {
            for (NSUInteger i= 0; i < queueCount; i++) {
                queues[i] = dispatch_queue_create("com.Hannb.download", DISPATCH_QUEUE_SERIAL);
...
- (void)downloadChapterWith:(YDownloadModel *)downloadM {
    ...
    downloadM.lastTask = [_netManager getWithAPIType:MBAPITypeChapterContent parameter:chapterM.link success:^(id response) {
